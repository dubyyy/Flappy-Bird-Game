...
// Store a snapshot of the game state for pause
    ...
    if (paused) {
      ...
      pausedSnapshot = {
        bird: {
          ...
          y: bird.y,
          angle: bird.angle,
          velocity_x: bird.velocity_x,
          velocity_y: bird.velocity_y,
          flipV: bird.flipV,
          flipH: bird.flipH,
        ...
        pipes: pipes.map(function(pipe) {
          return {
            ...
            y: pipe.y,
            angle: pipe.angle,
            velocity_x: pipe.velocity_x,
            velocity_y: pipe.velocity_y,
            flipV: pipe.flipV,
            flipH: pipe.flipH,
...
function resizeCanvas() {
  // Get the CSS size
  const style = window.getComputedStyle(myCanvas);
  let width = parseInt(style.width);
  let height = parseInt(style.height);
  // Set canvas internal size to match CSS
  myCanvas.width = width;
  myCanvas.height = height;
  // If bird exists, reposition it
...
function drawScore() {
  ...
  ctx.lineWidth = 4;
  ctx.textAlign = "center";
  ctx.strokeText(score, myCanvas.width / 2, 55);
  ctx.fillText(score, myCanvas.width / 2, 55);
}

function updateGame() {
  // Only update game state if not paused and running
...
function Do_a_Frame() {
  ...
  if (!paused) {
    ...
    // Draw frozen snapshot scene
    if (pausedSnapshot) {
      // Draw pipes from snapshot
      for (var i = 0; i < pausedSnapshot.pipes.length; i++) {
        var p = pausedSnapshot.pipes[i];
        ctx.save();
        ctx.translate(p.x + (p.MyImg.width ? p.MyImg.width/2 : 0), p.y + (p.MyImg.height ? p.MyImg.height/2 : 0));
        ctx.rotate((p.angle * Math.PI) / 180);
        if (p.flipV) ctx.scale(1, -1);
        if (p.flipH) ctx.scale(-1, 1);
        if (p.visible)
          ctx.drawImage(p.MyImg, -(p.MyImg.width ? p.MyImg.width/2 : 0), -(p.MyImg.height ? p.MyImg.height/2 : 0));
        ctx.restore();
      }
      // Draw bird from snapshot
      var b = pausedSnapshot.bird;
      ctx.save();
      ctx.translate(b.x + (b.MyImg.width ? b.MyImg.width/2 : 0), b.y + (b.MyImg.height ? b.MyImg.height/2 : 0));
      ctx.rotate((b.angle * Math.PI) / 180);
      if (b.flipV) ctx.scale(1, -1);
      if (b.flipH) ctx.scale(-1, 1);
      if (b.visible)
        ctx.drawImage(b.MyImg, -(b.MyImg.width ? b.MyImg.width/2 : 0), -(b.MyImg.height ? b.MyImg.height/2 : 0));
      ctx.restore();
      display_bar_running_along_bottom();
      drawScore();
    }
  ...
  if (paused && game_mode === "running") {
    ...
    ctx.fillStyle = "#fff";
    ctx.textAlign = "center";
    ctx.fillText("Paused", myCanvas.width / 2, myCanvas.height / 2);
    ctx.restore();
  }
}
